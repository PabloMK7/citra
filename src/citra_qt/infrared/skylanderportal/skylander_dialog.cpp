...
CreateSkylanderDialog::CreateSkylanderDialog(QWidget* parent) : QDialog(parent) {
    ...
    connect(btn_create, &QAbstractButton::clicked, this, [=, this]() {
        ...
        if (found_sky != list_skylanders.end()) {
            ...
            predef_name +=
                QString(QString::fromStdString("Unknown(%1 %2).sky")).arg(sky_id).arg(sky_var);
        }
        ...
        FileUtil::IOFile sky_file(file_path.toStdString(), "wb");
        if (!sky_file) {
        ...
        // Set checksum
        u16 crc = skylander_crc16(0xFFFF, data, 0x1E);
        memcpy(&data[0x1E], &crc, 2);

        sky_file.Seek(0, SEEK_SET);
        if (sky_file.WriteBytes(buf.data(), buf.size()) != buf.size()) {
            throw std::runtime_error("Could not write to file " + file_path.toStdString());
        }
        sky_file.Close();
...
QString CreateSkylanderDialog::get_file_path() const {
    return file_path;
}
...
SkylanderPortalWindow* SkylanderPortalWindow::get_dlg(QWidget* parent) {
    if (inst == nullptr)
        inst = new SkylanderPortalWindow(parent);

    return inst;
}

void SkylanderPortalWindow::clear_skylander(u8 slot) {
    if (auto slot_infos = sky_slots[slot]) {
        auto [cur_slot, id, var] = slot_infos.value();
        Service::IR::g_skyportal.RemoveSkylander(cur_slot);
        sky_slots[slot] = {};
        update_edits();
    }
}

void SkylanderPortalWindow::create_skylander(u8 slot) {
    CreateSkylanderDialog create_dlg(this);
    if (create_dlg.exec() == Accepted) {
        LOG_INFO(Service_IR, "{}", create_dlg.get_file_path().toStdString());
...
void SkylanderPortalWindow::load_skylander_path(u8 slot, const QString& path) {
    FileUtil::IOFile sky_file(path.toStdString(), "rb");
    if (!sky_file.IsGood()) {
        QMessageBox::warning(
    ...
    sky_file.Seek(0, SEEK_SET);
    if (sky_file.ReadBytes(data.data(), data.size()) != data.size()) {
        QMessageBox::warning(
    ...
    memcpy(&sky_id, &data[0x10], sizeof(sky_id));
    memcpy(&sky_var, &data[0x1C], sizeof(sky_var));

    u8 portal_slot = Service::IR::g_skyportal.LoadSkylander(data.data(), std::move(sky_file));
    sky_slots[slot] = std::tuple(portal_slot, sky_id, sky_var);

    update_edits();
}

void SkylanderPortalWindow::update_edits() {
    for (auto i = 0; i < UI_SKY_NUM; i++) {
