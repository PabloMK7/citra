...
static FORCE_INLINE void* AllocBuffer(size_t size) {
    size_t start = trans_cache_buf_top;
    size_t end = trans_cache_buf_top + size;
    ASSERT_MSG(end <= TRANS_CACHE_SIZE, "Translation cache is full!");
...
static ARM_INST_PTR INTERPRETER_TRANSLATE(bbl)(unsigned int inst, int index)
{
    ...
    //     inst_base->br |= TransExtData::COND;
